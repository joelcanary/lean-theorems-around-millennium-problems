...
| `HodgeMathlib.lean` | **Noether–Lefschetz window = OEIS [A005581](https://oeis.org/A005581):** for surfaces of degree `d` in `P^3`, the gap between the geometric genus `p_g(d) = C(d−1,3)` and the Green–Voisin codimension bound `d−3` equals `A005581(d−3) = (d−4)(d−3)(d+1)/6` for every integer `d` (`ventana_es_oeis_A005581`, stated without division as `(d−1)(d−2)(d−3) − 6(d−3) = (d−4)(d−3)(d+1)`). First observed numerically (`d ≤ 200`), then closed for all `d`. | Verified (`ring`). Preprint: [10.5281/zenodo.21535860](https://doi.org/10.5281/zenodo.21535860). |
| `Panoptic*.lean` | Exact theory of the Panoptic Quality metric used in the IEEE BigData Cup 2026 solar-filament challenge: when a duplicate raises the score (`pq_dup_iff`, `dup_iff`), no reweighting removes the defect (`ningun_peso_lo_arregla`), the candidate rule `p·j > PQ/2` (`pq_candidato_iff`) and its mirror for removal `p·j < PQ/2` (`pq_quitar_iff`), the two-annotator ceiling (`techo_dos_anotadores`), and that the threshold `1/2` is exact (`umbral_exacto`). | Verified. |
